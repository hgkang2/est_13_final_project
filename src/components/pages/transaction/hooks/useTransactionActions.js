...
export const useTransactionActions = ({
  ...
  // 저장/수정/삭제가 이미 진행 중이면 같은 작업을 다시 시작하지 않음
  const runMutation = async callback => {
    if (isMutatingRef.current) return;

    // 이미 진행 중인 상세 조회가 있다면 무효화
  ...
  const handleOpenDetail = async transaction => {
    ...
    // 그 사이 다른 거래 상세 요청이 시작됐다면 이전 결과 폐기
    ...
    if (attachmentData?.storage_path) {
      ...
      // signed URL 생성 중 다른 상세 요청이 시작된 경우도 폐기
      if (requestId !== detailRequestIdRef.current) return;
